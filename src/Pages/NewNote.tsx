...
const ReactSelect = ({SelectedTags,SetSelectedTags,Availabletags,OnAddtag}:ReactSelectProps) =>{
    return(
        ...
        <CreatableReactSelect  onCreateOption={label=>{const newtag = {id:uuidV4(),label};OnAddtag(newtag);SetSelectedTags(prev => [...prev,newtag])}} options={Availabletags.map(tag=>{return {value:tag.id,label:tag.label}})} className='w-[30rem]  text-gray-900 bg-gray-50 rounded-lg text-md font-light border border-gray-300 focus:ring-blue-500 focus:border-blue-500 placeholder:font-light   font-label  ' value={SelectedTags.map(tags => { return {label:tags.label,value:tags.id}} )} onChange={(tags => {SetSelectedTags(tags.map(tag => { return { label:tag.label , id:tag.value} }))  
        })} isMulti/>
        </>
    )
}
...
<textarea required style={{resize:"none"}}  ref={TitleRef}  rows={1} className=" p-2.5 w-[30rem]  text-lg placeholder:text-md text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500    font-pixel " placeholder="Title"></textarea>
</div>
 <div>
...
<textarea required ref={BodyRef}  rows={15} className="font-pixel text-lg block p-2.5 w-full  text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500   " placeholder="Write your thoughts here..."></textarea>
</div>
